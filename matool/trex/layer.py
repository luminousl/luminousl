...
class Layer:
    ...
    def tooltip_simple(self):
        tip = ""
        for key in ['Name', 'LayerType', 'Activation']:
            if key in self.raw_dict.keys():
                tip += f"{key}:{self.raw_dict[key]}\\n"
        return tip
...
def fold_no_ops(layers: List, bindings: List) -> List:
    ...
    ignore_ops = ['NoOp']
    # ignore_ops = ['NoOp', "shape_call", "wait", "signal"]
    ret = [layer for layer in ret.values() if layer.type not in ignore_ops]
    return ret
