...
def log_map(value, a, b, c, d, log_min=0.3, log_max=3):
    # 确保 a 不等于 b，以避免除以零错误
    if a == b:
        b = a + 1
        # raise ValueError("Invalid input: a must be different from b.")
    # linear map :      value -> x 
    # x = linear_map(value, a, b, math.exp(log_min), math.exp(log_max))
    x = linear_map(value, a, b, log_min, log_max)
    # log map:          y = ln(x)  
    y = math.log(x)
    # linear map :      y -> y' 
    # result = linear_map(y, log_min, log_max, c , d)
    result = linear_map(y, math.log(log_min), math.log(log_max), c , d)
...
def render_dot(dot_graph: Digraph, engine_name: str, output_format: str, view=False):
    ...
    dot_graph.render(outfile=output_fname, view=view, overwrite_source=False)
    return output_fname
...
def layer_node_configurable_renderer(
    ...
    min_op_lat: float=0,
    max_op_lat: float=1,
    min_op_height: float=20,
    max_op_height: float=60,
) -> str:
    ...
    def html_tbl(rows: List[str]):
        def html_tbl_row(row_content, bold:bool, color: str=None, height: float=None):
            ...
            # return row
            if color:
                row = f"<TR><TD HEIGHT=\"{height}\" BGCOLOR=\"{color}\">{row_content}</TD></TR>"
            else:
                row = f"<TR><TD HEIGHT=\"{height}\">{row_content}</TD></TR>"
            return row
        ...
        # height_0 = linear_map(latency, 
        #     min_op_lat, max_op_lat,
        #     min_op_height, max_op_height)
        if latency:
            height = log_map(latency, 
                min_op_lat, max_op_lat,
                min_op_height, max_op_height)
        else:
            height = log_map(min_op_lat, 
                min_op_lat, max_op_lat,
                min_op_height, max_op_height)

        for i, row in enumerate(rows):
            tbl += html_tbl_row(row[0], i==0, 
...
def layer_type_formatter_simple(layer: Layer):
    """Format Dot nodes by layer type"""
...
class DotGraph(object):
    ...
    def __init__(self,
        ...
        self.min_op_lat = min(plan.df['latency.avg_time'])
        self.max_op_lat = max(plan.df['latency.avg_time'])
        self.min_op_height = min_op_height
        self.max_op_height = max_op_height

        node_name_2_node_id = {}
        self.__add_dot_region_nodes(plan_graph, node_name_2_node_id)
        self.__add_dot_layer_nodes(plan, plan_graph, node_name_2_node_id)
        t_sizes = [edge.tensor.size_bytes for edge in plan_graph.edges_list]
        self.min_tensor_size = min(t_sizes)
        self.mean_tensor_size = sum(t_sizes) / len(t_sizes)
        self.max_tensor_size = max(t_sizes)
        ...
        for edge in plan_graph.edges_list:
            ...
            indices1 = plan.df.index[plan.df['Name'] == edge.src.layer_name].tolist()
            indices2 = plan.df.index[plan.df['Name'] == edge.dst.layer_name].tolist()
            indices1 = str(indices1[0]) if indices1 else "X"
            indices2 = str(indices2[0]) if indices2 else "X"
            edge_ind_msg = indices1 + "-" + indices2
            # print("TESTING: ", indices1, indices2, src_id)
            self.edge_record.append(edge_ind_msg)
            shape = self.get_output_shape_for_edge(edge.src.layer_name, 
                                                       edge.dst.layer_name)
            self.__create_dot_edge(src_id, dst_id, edge.tensor, edge.region_gen, edge_ind_msg,
                                   out_shape=shape)
    ...
    def __create_dot_layer_node(
        ...
        self.dot.node(
            ...
            layer_node_renderer(
                ...
                min_op_lat=self.min_op_lat,
                max_op_lat=self.max_op_lat,
                min_op_height=self.min_op_height,
                max_op_height=self.max_op_height,
                ),
                **formatting)
        
    def get_output_shape_for_edge(self, src, end):
        src = src[:-2] if src.endswith('.0') else src
        end = end[:-2] if end.endswith('.0') else end
        # print('aaa\t', src)
        # print('ddd\t', end)
        try:
            df_dst = self.plan.df[self.plan.df['Name']== end].reset_index()
            for inp in df_dst.Inputs[0]:
                if inp['Name'] == src:
                    return inp['Dimensions']
        except:
            return None

    def __create_dot_edge(self, src, end, tensor, region_gen, edge_ind_msg, out_shape=None):
        ...
        desc = []
        desc.append(edge_ind_msg)
        if self.display_edge_name:
            desc.append(tensor.name)
        if self.display_edge_details:
            if out_shape:
                tip = "\\n".join((str(out_shape), tensor.format,))
                desc.append(tip)
            else:
                desc.append(tensor.tooltip())
        desc_str = "\n".join(desc)
        tensor_size = np.prod(out_shape) * tensor.data_size \
            if out_shape else tensor.size_bytes
        
         # 使用对数尺度映射 byte 到 penwidth
        # min_byte = 0
        # max_byte = 5000000
        ## log projection
        # log_min_byte = math.log(min_byte)
        # log_max_byte = math.log(max_byte)
        # log_byte = math.log(tensor.size_bytes) if hasattr(tensor, 'size_bytes') else log_min_byte
        # normalized_byte = (log_byte - log_min_byte) / (log_max_byte - log_min_byte)
        ## linear projection
        
        penwidth = linear_map(tensor_size, #tensor.size_bytes
            self.min_tensor_size, 5*self.mean_tensor_size,
            self.min_edge_width, self.max_edge_width)
        
        # print('From', src)
        # print('\t To', end)
        # print('\t\t', penwidth, '\t', tensor.size_bytes, '\t', )
        penwidth = str(penwidth)

        # normalized_byte = (tensor.size_bytes - min_byte) / (max_byte - min_byte)

        # penwidth_factor = 20.0  # 调整这个因子以控制粗细
        # penwidth = str(normalized_byte * penwidth_factor)
        # print(tensor.name, tensor.size_bytes, tensor.precision, penwidth)
        self.dot.edge(src, end, desc_str, color=edge_color, 
            penwidth=penwidth, arrowhead='none')


def to_dot(*args, **kwargs) -> Digraph:
    """Convert plan-graph to dot format"""
    g = DotGraph(*args, **kwargs)
    return g.dot, g.edge_record
