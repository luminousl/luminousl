...
def group_count_multi(df, grouping_attr: list):
    grp = df.groupby(grouping_attr).size().reset_index(name='count')
    return grp
...
def group_sum_attr_multi(df, grouping_attr:list, reduced_attr:list):
    grp = df.groupby(grouping_attr).sum()[reduced_attr].reset_index()
    return grp
