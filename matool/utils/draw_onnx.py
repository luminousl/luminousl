...
# 自定义
color_map_def_1=defaultdict(lambda: UNKNOWN_KEY_COLOR,  {
        'Conv':                 "#4682B4", 
...
color_map_def_2=defaultdict(lambda: UNKNOWN_KEY_COLOR,  {
        # 矩阵操作
        'Conv':                 "#4682B4", 
        'ConvTranspose':        "#7B68EE",
        'Gemm':                 "#1E90FF",
        ...
        # 池化
        'AveragePool':          "#3CB371",
        'GlobalAveragePool':    "#3CB371",
        'GlobalLpPool':         "#3CB371",
        'GlobalMaxPool':        "#3CB371",
        'LpPool':               "#3CB371",
        'MaxPool':              "#3CB371",
        # 激活函数
        'Relu':                 "#903749", 
        ...
        'TopK':                 "#DA70D6", 
        'Concat':               "#4c221b",
        'QuantizeLinear':       "#6B8E23",
        # Plugin
        'bev_pool_v2':          "red", 
    })

#----------------------------------------------------------------------
def get_color_from_map(node_type):
    color_map=color_map_def_2
    color = color_map[node_type]
    return color


def makeDot(model, addIndex = False, addName = True):
    ...
    for index, node in enumerate(ingraph.node):
        ...
        if addName and node.name:
            labels.append("Name: " + node.name)
        ...
        print(node)
        print(">>>>>>>>>>>>>>>>>>>>>>>>>")
        # labels.append(get_tensor_shape(node))
        #----------
        ...
        color = get_color_from_map(node.op_type)
        # print(color)
...
if __name__ == '__main__':
    ...
    fin = "/media/models/radar_bev/dynamic_model_concat.onnx"
    outputFname = "/media/models/radar_bev/dynamic_model_concat.onnx.dot"

    fin = "/data/models/20231101/onnx/pnc/pred_model.onnx"
    outputFname = "/data/models/20231101/onnx/pnc/pred_model.dot"

    
    outputFormat = outputFname.split('.')[-1].lower()
    svg_file = outputFname.split('.')[0] + '.svg'
    svg_file2 = outputFname.split('.')[0] + '_name.svg'
    svg_file3 = outputFname.split('.')[0] + '_shape.svg'
    ...
    # outgraph.write_svg(svg_file)
    # outgraph2.write_svg(svg_file2)

    makeDot(model_with_shape, addName=False).write_svg(svg_file)
    makeDot(model, addName=False).write_svg(svg_file2)
    # makeDot(model_with_shape, addName=True).write_svg(svg_file3)