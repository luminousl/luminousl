...
def get_engine_path(args: Dict, add_suffix: bool):
    if add_suffix:
        trt_path = args.input
        trt_fname = os.path.basename(trt_path)
        outdir = args.outdir
        engine_path = os.path.join(outdir, trt_fname)
    else:
...
def generate_engine_svg(args: Dict, add_suffix: bool) -> bool:
    ...
    try:
        ...
        print(f"Generating graph diagram: {graph_json_fname}.svg")
...
def process_engine(
    ...
    success = True
    if build:
        print("Input file is trt engine, no need to build")
...
def parse_args(args):
    # assert args.input.endswith(".engine"), f"输入文件错误，须为*.engine"
    assert os.path.isfile(args.input), f"输入文件{args.input}不存在"

    input_dir = os.path.dirname(args.input)
    model_name = os.path.basename(args.input)
    if not args.outdir:
        args.outdir = os.path.join(input_dir, model_name + '_profile')
...
def trt2profile(onnx, trt_args, outdir='', trtexec=None):
...
if __name__ == "__main__":
    ...
    parser.add_argument('input', help="input trt engine file path")
    ...
    parser.add_argument('--build-engine', '-b', action='store_true', default=False,
