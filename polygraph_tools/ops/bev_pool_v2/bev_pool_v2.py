...
if torch.cuda.get_device_name() == "NVIDIA Graphics Device MIG 1g.10gb" or torch.cuda.get_device_name() == "NVIDIA Graphics Device MIG 2g.20gb" or torch.cuda.get_device_name() == "NVIDIA A100-SXM4-80GB":
    from . import bev_pool_v2_ext
else:
    import bev_pool_v2_ext

__all__ = ['bev_pool_v2', 'TRTBEVPoolv2', 'bev_pool_v2_deploy']
...
def bev_pool_v2_deploy(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                bev_feat_shape, interval_starts, interval_lengths):
...
class TRTBEVPoolv2(torch.autograd.Function):
    ...
    def forward(g,
        ...
        # bev_feat_shape = (int(depth.shape[0]), 1, out_height, out_width,
        #                   int(feat.shape[-1]))  # (B, Z, Y, X, C) # 1, 1, 80, 200, 80
        bev_feat_shape = (int(depth.shape[0]), 1, out_height, out_width,
                          int(feat.shape[-1]))  # (B, Z, Y, X, C) # 1, 1, 80, 200, 80
        # for deploy
        # torch.save(feat, "inputs/feat_{}.pt".format(0))
        bev_feat = bev_pool_v2_deploy(depth, feat, ranks_depth, ranks_feat, ranks_bev,
                               bev_feat_shape, interval_starts,
                               interval_lengths) # 1, 1, 80, 200, 80
        return bev_feat
