...
def get_int8_calibrator(calib_cache, calib_data, max_calib_size, preprocess_func_name, calib_batch_size, input_info):
    ...
    int8_calibrator = FakeInputCalibrator(calibration_files=calib_files,
...
class FakeInputCalibrator(trt.IInt8EntropyCalibrator2):
    ...
    def __init__(self, calibration_files=[], batch_size=32, input_info={},
        ...
        self.batch = {k:np.zeros((self.batch_size, *v['shape'][1:]), dtype=v['dtype']) for k,v in self.input_info.items()}
        self.device_input = {k: cuda.mem_alloc(self.batch[k].nbytes) for k in self.input_info.keys()}
    ...
    def get_batch(self, names):
        try:
            ...
            for name in names:
                ...
                batch_input.append(int(self.device_input[name]))
