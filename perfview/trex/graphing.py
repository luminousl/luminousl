    ...
    def __init__(self, plan: EnginePlan, display_forking_regions: bool):
        def get_adjacency_lists():
            ...
            for edge in self.plan_graph.edges_list:
                ...
                if edge.src.layer_name not in outputs_map:
                    outputs_map[edge.src.layer_name] = []
                outputs_map[edge.src.layer_name].append(edge_name)

                if edge.dst.layer_name not in inputs_map:
                    inputs_map[edge.dst.layer_name] = []
        ...
        def add_layer_nodes():
            for layer_id, layer_node in enumerate(self.plan_graph.layer_nodes):
                layer = layer_node.layer
                # try:
                if layer.name == "__myl_MulMinMaxRouConMovCas_myl0_1":
                    print(layer, inputs_map)
                self.__add_layer_node(layer_id, layer, inputs_map.get(layer.name, []), outputs_map.get(layer.name, []))
        ...
        def add_attributes(layer, node_def):
            for key, value in sorted(layer.raw_dict.items()):
                if key not in [
                    ...
                    'Outputs', 'Name', 'name', 'ParameterType', 'LayerName', "Params"]:
                    try:
            ...
            for latency_type in latency_types:
                latency = _get_latency(self.plan, layer, latency_type=latency_type)
                node_def.attribute.append(onnx.helper.make_attribute(f"latency_{latency_type}", latency))
